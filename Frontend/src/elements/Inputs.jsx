...
export function TextInput({label, resetInputs, handleNewSubmit}){
    ...
    return (
        <label htmlFor={`${label.split(' ').join('').toLowerCase()}`} 
            id='label-text'
            className="appearance-none block w-full p-3 text-gray-700 bg-white 
                    border border-gray-300 rounded-lg shadow-md focus:outline-none focus:ring-2
        ...
        >
            <input name={`${label.split(' ').join('').toLowerCase()}`} 
                ...
                type="text" 
                className='border-2 border-transparent focus:border-orange-200 focus:outline-none 
