...
app.use(session({
    ...
    store: new session.MemoryStore(), // Usa un almacenamiento en memoria
    rolling: true, // Actualiza automáticamente el token de sesión cada vez que se hace una solicitud
...
app.use((req, res, next) => {
    ...
    next(); // Llama al siguiente middleware/ruta
});
...
app.listen(PORT, async () => {
    ...
    // Prueba la conexión después de que el servidor esté en ejecución
    try {
        const connection = await poolDB.getConnection();
        console.log('Conexión exitosa al pool');

        // Libera la conexión después de la prueba
        connection.release(); 
    } catch (err) {
